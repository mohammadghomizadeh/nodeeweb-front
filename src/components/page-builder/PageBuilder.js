...
export function ShowElement(p) {
  ...
  switch (name) {
    ...
    case "radio":
      return <RADIO element={element} content={content} params={params}/>;
    case "select":
      return <SELECT element={element} content={content} params={params}/>;
    case "TEXTBOX":
...
export function RADIO({element}) {
  console.log(element)
  let {type, components, classes, settings,id} = element;
  let {general} = settings;
  let {fields} = general;
  let {label,name,value,text, iconFont, direction, link, display, showInDesktop, showInMobile, target = "_blank",inputClassName,wrapperClassName,labelClassName} = fields;
  let style = setStyles({...fields, direction: direction, display: display});
  if (link) {
  ...
  return <div style={style}
      ...
      <div className={wrapperClassName}>
        <input className={inputClassName} type="radio" name={name} value={value}
               id={id}/>
    ...
    {/*{(iconFont && Icons[iconFont]) && <span>{React.createElement(Icons[iconFont])}</span>}*/}
    {/*<span dangerouslySetInnerHTML={{__html: text}}/>*/}
    {/*<input type="radio" dangerouslySetInnerHTML={{__html: text}}/>*/}
...
export function SELECT({element}) {
  ...
  let {general} = settings;
  let {fields} = general;
  let {options,placeholder,text, direction, display, showInDesktop, showInMobile, inputClassName,wrapperClassName,labelClassName} = fields;
  let style = setStyles({...fields, direction: direction, display: display});
  return <div style={style}
              className={(showInDesktop ? ' showInDesktop ' : '') + (showInMobile ? ' showInMobile ' : '')}>
    <div className={wrapperClassName}>
      <select className={inputClassName} aria-label={text}>
